-- RPC para Pedidos (auditoría Supabase - sección 2)
-- Ejecutar DESPUÉS de ROLES_USERS_RPC.sql (usa user_has_permission).
-- Usa permisos por rol (create_orders, complete_orders) para que roles personalizados funcionen.
-- 1. create_order: quien tenga create_orders O rol tienda/admin/almacen (retrocompat)
-- 2. complete_order: quien tenga complete_orders O rol almacen/admin (retrocompat)
...
BEGIN
  ...
  SELECT user_has_permission(v_uid, 'create_orders')
  ...
  IF NOT COALESCE(v_can_create, false) THEN
    RAISE EXCEPTION 'No tienes permiso para crear pedidos (create_orders o rol tienda/admin/almacen)';
  END IF;
  ...
  INSERT INTO orders (created_by, restaurant_id, status, notes)
  VALUES (v_uid, p_restaurant_id, 'pending', NULLIF(trim(p_notes), ''))
...
BEGIN
  ...
  IF v_order.status NOT IN ('pending', 'processing') THEN
    RAISE EXCEPTION 'Solo se pueden completar pedidos pendientes o en proceso';
  END IF;

  -- Quien creó el pedido, o quien tenga complete_orders, o rol almacen/admin
  IF v_order.created_by != v_uid
     AND NOT user_has_permission(v_uid, 'complete_orders')
     AND NOT EXISTS (
       SELECT 1 FROM user_profiles up
       WHERE up.id = v_uid AND up.role_name IN ('almacen', 'admin')
     ) THEN
    RAISE EXCEPTION 'No tienes permiso para completar este pedido (complete_orders o rol almacen/admin)';
  END IF;

  FOR v_item IN
    SELECT oi.id, oi.product_id, oi.product_name,
           COALESCE(NULLIF(oi.quantity_sent, 0), oi.quantity_requested, oi.quantity, 0) AS qty
    FROM order_items oi
